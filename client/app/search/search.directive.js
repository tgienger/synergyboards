'use strict';

angular.module('synergyApp')
  .directive('synergySearch', function ($timeout) {
    return {
      restrict: 'E',
      transclude: true,
      replace: true,
      templateUrl: 'app/search/search.directive.html',
      scope: {
          search: '&',
          select: '&',
          term: '=',
          searchCollapse: '=',
          items: '=',
          searching: '=',
      },
      link: function (scope, element, attrs, controller) {

          var $input = element.find('form > div > input'),
              $list = element.find('> div');

          $input.bind('focus', function(event) {
              scope.$apply(function() {scope.focused = true;});
          });

          $input.bind('blur', function(event) {
              scope.searchCollapse = true;
              scope.posts = [];
              scope.users = [];
              scope.$apply(function() {scope.focused = false;});
          });

          $list.bind('mouseover', function(event) {
              scope.$apply(function() {scope.mousedOver = true;});
          });

          $list.bind('mouseleave', function(event) {
              scope.$apply(function() {scope.mousedOver = false;});
          });

          $input.bind('keyup', function(event) {

              if (event.keyCode === 9 || event.keyCode === 13) {
                  scope.$apply(function() {controller.selectActive();});
              }

              if (event.keyCode === 27) {
                  scope.$apply(function() {
                      if (scope.hide) {
                          scope.searchCollapse = true;
                      } else {
                          scope.hide = true;
                      }
                  });
              }

          });

          $input.bind('keydown', function(event) {

              if (event.keyCode === 9 || event.keyCode === 13 || event.keyCode === 27) {
                  event.preventDefault();
              }

              if (event.keyCode !== 27) {
                  scope.hide = false;
              }

              if (event.keyCode === 40) {
                  event.preventDefault();
                  scope.$apply(function() {controller.activateNextItem();});
              }

              if (event.keyCode === 38) {
                  event.preventDefault();
                  scope.$apply(function() {controller.activatePreviousItem();});
              }
          });

          scope.$watch('items', function(items) {
              controller.activate(items.length ? items[0] : null);
          });

          scope.$watch('focused', function(focused) {
              if (focused) {
                  $timeout(function() {$input.focus();}, 0, false);
              }
          });

          scope.$watch('isVisible()', function(visible) {
              if (visible) {
                  $list.css({display: 'block'});
              } else {
                  $list.css({display: 'none'});
              }
          });

          var input = angular.element('#search-input');
          scope.$watch('searchCollapse', function(n, o) {
              if (o === true) {
                  scope.hide = false;
                  $timeout(function() {input.focus();});
              }
          });
      },
      controller: ['$scope', function($scope) {

          var typingTimer;
          $scope.items = [];
          $scope.hide = false;

          this.activate = function(item) {
              $scope.active = item;
          };

          this.activateNextItem = function() {
              var index = $scope.items.indexOf($scope.active);
              this.activate($scope.items[(index+1) % $scope.items.length]);
          };

          this.activatePreviousItem = function() {
            var index = $scope.items.indexOf($scope.active);
            this.activate($scope.items[index === 0 ? $scope.items.length - 1 : index - 1]);
          };

          this.isActive = function(item) {
              return $scope.active === item;
          };

          this.selectActive = function() {
            this.select($scope.active);
          };

          this.select = function(item) {
              $scope.hide = true;
              $scope.focused = true;
              $scope.select({item:item});
          };

          $scope.isVisible = function() {
            return !$scope.hide && ($scope.focused || $scope.mousedOver) && $scope.items.length;
          };

          $scope.query = function() {
              $timeout.cancel( typingTimer );

              typingTimer = $timeout(function() {
                  $scope.search( {term:$scope.term} );
              }, 400);
          };

      }]
    };
  })
  .directive('selectedItem', function() {
      return {
          require: '^synergySearch',
          link: function(scope, element, attrs, controller) {
              var item = scope.$eval(attrs.selectedItem);

              scope.$watch(function() {
                  return controller.isActive(item);
              }, function(active) {
                  if (active) {
                      element.addClass('active');
                  } else {
                      element.removeClass('active');
                  }
              });

              element.bind('mouseenter', function(e) {
                  scope.$apply(function() {controller.activate(item);});
              });

              element.bind('click', function(e) {
                  scope.$apply(function() {controller.select(item);});
              });
          }
      };
  });
